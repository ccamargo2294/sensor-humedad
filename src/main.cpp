...
#define WIFI_SSID "Sweet home Medellin"
...
void onMqttConnect(bool sessionPresent) {
  ...
  mqttClient.subscribe("IUE/habitacion/riego", 2);
  mqttClient.subscribe("IUE/todos/riego", 2);
...
void onMqttMessage(char* topic, char* payload, AsyncMqttClientMessageProperties properties, size_t len, size_t index, size_t total) {
  ...
  if(String(topic).indexOf("riego") > 0) {
    Serial.print("Comando de riego:");
    int command = String(payload).toInt();
